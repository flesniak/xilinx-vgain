...
/*void* memset(void* data, unsigned char value, int size) {
...
void blur(unsigned char* data, int xsize, int ysize) {
  for(int y = 1; y < ysize-1; y+=1)
    for(int x = 1; x < xsize-1; x+=1)
      *(data+y*xsize+x) = ((int)*(data+(y+1)*xsize+x+1) + *(data+(y+1)*xsize+x-1) + *(data+(y-1)*xsize+x+1) + *(data+(y-1)*xsize+x-1) + *(data+y*xsize+x)) / 5;
}
...
void getImage(void* dst, int* xsize, int* ysize, void* baseAddr, int nBuffers, int offset) {
  ...
  *xsize = VIN_VMEM_WIDTH;
  *ysize = VIN_VMEM_HEIGHT;
  //blur(dst, *xsize, *ysize);
}

#include "../../xilinx-dvi/dvi-mem.h"

//write image to DVI peripheral and convert to 6-bit colorspace
void putImage(const void* src, int xsize, int ysize) {
  unsigned int* fbOut = (unsigned int*)DVI_VMEM_ADDRESS;
  unsigned char* csrc = (unsigned char*)src;

  for( unsigned int y = 0; y < ysize; y++ )
    for( unsigned int x = 0; x < xsize; x++ ) {
      unsigned int out = *(csrc+y*xsize+x);
      *(fbOut+y*DVI_VMEM_WIDTH+x) = ((out | (out << 8) | (out << 16)) >> 2) & 0x003F3F3F;
    }
}

unsigned char in[VIN_VMEM_WIDTH*VIN_VMEM_HEIGHT];
unsigned char mid[VIN_VMEM_WIDTH*VIN_VMEM_HEIGHT];
int r[VIN_VMEM_WIDTH*VIN_VMEM_HEIGHT];
unsigned char bpt[516];
//#include "picture.h"

int main() {
//unused due to static memory allocation
//uchar  *in, *bp=0, *mid;
//int    *r;
uchar *bp = bpt;
...
int    mode           = 1, /* 0: smoothing, 1: edges, 2: corners */
       principle      = 0, /* principle */
       thin_post_proc = 0, /* thinning post processing */
       three_by_three = 0, /* 3x3 flat mask */
       drawing_mode   = 0, /* simple drawing mode */
       susan_quick    = 0, /* quick susan mask */
       bt             = 20, /* brightness threshold */
       max_no_corners = 1850,
       max_no_edges   = 2650,
       x_size         = 640,
       y_size         = 480;
float  dt             = 4.0; /* distance threshold */
CORNER_LIST corner_list;
